...
\end{equation}
The integral may be evaluated by the SPA.
Applying high-frequency approximations the gradient of the sound field reads
\begin{equation}
\nabla P(\vx) \approx \ti \phi_P(\vx) P(\vx) = -\ti \vk_P^l(\vx) P(\vx),
\end{equation}
thus the phase function under consideration is
\begin{equation}
\phi(\vx,\vxo) \approx \frac{\pi}{2} + \phi_P(x_0,0,z) + \phi_G(x-x_0,y,z-z_0).
\end{equation}
The stationary position $x_0^*(x)$, $z_0^*(z)$ is found, where 
