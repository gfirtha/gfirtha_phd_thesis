...
One important fact is pointed out here: although having derived the above driving function in terms a forward an inverse spatial Fourier transform along a straight line, there is no restriction in the $y$-coordinate of the stationary point in \eqref{eq:spatial_sdm} due to the local approximations involved: the $y$-coordinate might be depend upon $x_0$. This means, that an arbitrary referencing curve may be defined as $\vxref(x,y)$, and the driving function can be calculated by finding the stationary positions along this curve, and by evaluating the driving functions in the stationary positions. The synthesis will result in amplitude correct synthesis along the reference curve. 

\subsection{Application example}
As a simple application consider the synthesis of a 2D/3D point source, referencing the synthesis to a circle around the virtual point source. For the sake of simplicity the source is located at $\vxs = [0,\ y_s,\ 0]^{\mathrm{T}}$. Along with the reference curve equation the stationary points are described by the following equations (since $k_{x,P}(x^*(x_0),y(x_0)) = k_{x,G}(x^*(x_0) - x_0,y(x_0)) $)
\begin{align}
k \frac{x^*(x_0)}{\sqrt{x^*(x_0)^2 + (y(x_0)-y_s)^2}} &= k \frac{x^*(x_0)-x_0}{\sqrt{(x^*(x_0)-x_0)^2 + y(x_0)^2}}, \\
x^*(x_0)^2 + (y(x_0)-y_s)^2     &= R_{\mathrm{ref}}^2.
\end{align}
The solution for the equations with taking the positive root into consideration is given by
\begin{align}
