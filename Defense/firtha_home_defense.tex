...
\frametitle{Thesis group IV: SDM of moving sources}
\begin{itemize}
\item Explicit moving source driving function:
\begin{itemize}
\item Found for Harmonic sources moving uniformly along a linear SSD

\item Proven to be equivalent with WFS under high frequency assumptions
\item Utilization: anti-aliasing strategy for moving sources
\end{itemize}
\end{itemize}
\end{frame}
...
\small
\begin{itemize}
\item Impulsive source, $v_0 = 200~\mathrm{m/s}$
\item Circular SSD, $N=120$ ($\Delta x \approx 10~\mathrm{cm}$)
\item Anti-aliasing (nonstationary) filtering: performed with high-resolution STFT 
\end{itemize}
\includemedia[
  width=1\linewidth,
  height=0.5091\linewidth, % 16:9
...
\begin{frame}
\frametitle{Thesis group III-IV: Synthesis of moving sources}
In the aspect of synthesizing dynamic sound scenes the synthesis of moving sources is of primary importance. I adapted the unified WFS framework to the synthesis of sound fields induced by moving point sources.
	\begin{itemize}
	\item 3D moving source WFS driving functions
	\item Time-domain 2.5D WFS driving functions
	\item Frequency-domain 2.5D WFS driving functions
	\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Synthesis of moving sources in the wavenumber domain}
For the case of a linear SSD and a parallel reference line the spatial Fourier transform of the involved functions (i.e. the target field, the SSD’s sound
field and the driving function) may be defined. In the wavenumber domain the synthesis problem can be formulated as a spectral multiplication,
and the driving functions are obtained via a spectral division, (hence the term Spectral Division Method (SDM)). I gave analytical expressions
for the required transforms for the case of uniformly moving virtual sources, resulting in the explicit reference solution.
	\begin{itemize}
	\item 2.5D moving source SDM driving functions
	\item Proof for WFS and SDM equivalence in the dynamic case
	\item Description and avoidance strategy for spatial aliasing in moving source case
	\end{itemize}
\end{frame}
