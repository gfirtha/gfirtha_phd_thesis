...
\end{thesis}
\begin{thesis}
I exploited the above physical interpretation to derive WFS driving functions for an arbitrary shaped SSD curve within the validity of the physical optics approximation of the Kirchhoff-Helmholtz integral.
\end{thesis}
\begin{thesis}
...
I introduced the \emph{referencing function}---an amplitude correction term in the driving functions---that controls the shape of the above reference curve.
...
a simple anti-aliasing criterion, that can be implemented by simple low-pass filtering of the loudspeaker driving signals \cite{Firtha2018_daga_a_booklet}.
\end{thesis}
\end{thesisgroup}
...
I verified that the analytic expression converges weakly to a Dirac-delta distribution for a sound source, moving along the direction of the Fourier-transform \cite{firtha2015sound_booklet} \end{thesis}
\begin{thesis} 
...
By using the given formulation spatial aliasing may be eliminated by simple low-pass filtering of the loudspeaker driving signals \cite{Firtha2018_daga_moving_source_booklet}.
\end{thesis}
\end{thesisgroup}
