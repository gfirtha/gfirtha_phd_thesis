...
Suppose a sound field, synthesized by a planar SSD located at $\vxo = [x_0,\ 0,\ z_0]^{\mathrm{T}}$, applying the planar WFS driving functions given implicitly by the \emph{3D Neumann Rayleigh integral}. By applying the high frequency gradient approximation for the target sound field the synthesized field reads
\begin{equation}
...
In order to reduce the problem dimensionality the SPA is applied to evaluate the integral vertically, along the $z_0$-dimension resulting in driving functions for a linear SSD.
In the followings we restrict our investigation to the plane $z=0$, termed the \emph{synthesis plane}---containing the linear SSD---,
and therefore $\vx = [x,\ y,\ 0]^{\mathrm{T}}$ denotes in-plane positions.
...
With these considerations the vertical stationary position is trivially located at $z_0^* = 0$---i.e. the lies on the linear SSD---, satisfying
\begin{equation}
\underbrace{k_z^P(x_0,0,0)}_{ = 0} + k_z^{G}(x - x_0,0,-z_0^*) = k \frac{z_0^*}{|\vx-\vxo^*|} =0,
\end{equation}
based on the physical interpretation for the SPA of the Rayleigh integral, presented in the previous section.
Hence it is ensured, that the propagation direction of the virtual sound field coincides with that of the linear SSD elements in the plane of synthesis.

Applying the SPA to the 3D Rayleigh integral results in a line integral representation of the synthesized field at the synthesis plane, termed the \emph{2.5D Neumann Rayleigh integral}\footnote{\color{red}{Refer to curvatures in order to give the sign of the second phase derivatives!!!}}
\begin{equation}
P(\vx,\omega) = \int_{-\infty}^{\infty} -2
...
\left. \frac{\partial}{\partial y} P(\vx,\omega) \right|_{\vx = \vxo}.
\end{equation}
The driving functions are however still dependent upon the receiver position $\vx$.
