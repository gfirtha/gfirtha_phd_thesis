...
As given in the previous chapters the general 3D Rayleigh integral, describing also the synthesized field using an infinite planar SSD located at 
$\vxo = \posvec{3}{x_0}{0}{z_0}$ reads
\begin{equation}
P(\vx,\omega) = \iint_{-\infty}^{\infty} - 2 \underbrace{ \left. \frac{\partial}{\partial y} P(\vxo,\omega) \right|_{y = 0}}_{ \approx -\ti k_y(\vxo) P(\vxo,\omega)}
...
\\
\nabla_{x_0,z_0} \left.
\left( \frac{\pi}{2}+ \phi_P(\vxo,\omega) + \phi_G(\vx-\vxo,\omega) \right) \right|_{\vxo^*} = 0
\end{multline}
holds.
