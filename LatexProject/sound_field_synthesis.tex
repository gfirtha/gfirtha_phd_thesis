...
The implicit solution for the SFS problem aims at the derivation of a single layer potential representation of the target sound field, containing the required SSD driving functions implicitly.
...
By reversing the causality choosing $\vxo$ as an independent parameter the driving functions can be extracted from \eqref{Eq:SFS_theory:25D_KI_appr} resulting in the \emph{unified 2.5D WFS driving functions}
...
In practical applications the set of the reference positions for all SSD elements form a prescribed continuous \emph{reference curve} $C_{\mathrm{ref}}$, so that the reference position for an SSD element is found at the intersection of the straight line, described by \eqref{Eq:SFS_theory:SSD_ref_positions} and the reference curve.
...
\importanteq{Temporal 2.5D point source driv. fun.}{
d(\vxo, t) =  -
\sqrt{\frac{1}{2\pi c}}
...
\end{equation}
The planar explicit driving functions are thus equivalent to the implicit solution, in a planar geometry provided by the Rayleigh integral.
...
As the geometry for the derivation of the 2.5D explicit driving functions assume an infinite linear distribution of secondary point sources, located at $\vxo = \posvec{3}{x_0}{0}{0}$.
...
\end{enumerate}
As the result of the derivation for a linear SSD located along $\vxo = \posvec{3}{x_0}{0}{0}$ the asymptotic SDM driving functions, expressed entirely in the spatial domain read as
