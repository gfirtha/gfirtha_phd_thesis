...
Given a \emph{target sound field} or the sound field of a \emph{virtual source} $P(\vx,\omega)$, the aim is to solve the above integral equation for $D(\vxo,\omega)$, so that the superposition of the secondary source's sound fields---the \emph{synthesized field}---equals to the target sound field. %\fscom{these definitions appear somewhere before, I'd suggest to define them earlier and reclaim them here.}
...
In practice it is often sufficient to restrict the reproduction to the $z=0$ plane, realizing a 2D contour of secondary sources.
...
In the following this chapter presents the approaches to solve the 3D and 2.5D SFS problem including physically based implicit and particularly mathematical explicit solutions. 
...
\end{equation}
or making use of the high frequency gradient approximation, by
...
In the special case of an infinite planar SSD surface located along the plane $y = y_0$, the Kirchhoff-Helmholtz integral degenerates to the Rayleigh I (Neumann) integral 
\fscom{this should be rewritten since an infinite planar SSD built with monopoles and dipoles is not the same as the SLP or DLP. maybe: the KHI degenerates to Rayleigh when considering only the target half space?}
representing the field of any source distribution located at $y<y_0$ in terms of a single layer potential, as discussed in Section \ref{Section:Theory:Rayleigh}.
...
\fscom{I'd avoid the term directivity here. Maybe its more appropriate to term this modal characteristics or so. I'd avoid non-homogeneous distribution either, describe more precisely what is meant.}
...
Substituting the Green's function into \eqref{Eq:Theory:2D_3D_WFS_driv_fun_2} yields the point source specific 3D WFS driving function\fscom{note that this is already HF approx}
\begin{equation}
...
\td \omega.
\end{equation}
By realizing that $\ti \omega \, S(\omega)$ describes the temporal derivative of the source time history \fscom{(i.e. filtering with a 6 dB/oct. highpass slope)} and exploiting the Fourier transform shift theorem, the time domain 3D WFS driving function for a virtual point source is obtained as
\begin{equation}
...
with both $\vx = \posvec{3}{x}{y}{0}$ and $\vxo = \posvec{3}{x_0}{y_0}{0}$ denoting in-plane positions in the following when 2.5D scenarios are considered.% and \fscom{HF directional gradient}.
...
\importanteq{WFS reference position}{
...
In practical applications the set of the reference positions for all SSD elements form a prescribed continuous \emph{reference curve} $C_{\mathrm{ref}}$, so that the reference position for an SSD element is found at the intersection of this straight line and the reference curve.
...
Generally speaking, the evaluation of the referencing function \eqref{Eq:SFS_theory:Referencing function} requires the definition of Green's function's principal radius, positioned at $\vxo$ and measured at the reference position.
For the special case of the Green's function this is given by the simple Euclidean distance of the two points.
Hence, the determination of the distance between the reference position and the corresponding SSD elements \fscom{replace all 'SSD elements' with 'secondary sources' as in IEEE18?} is required by solving equation \eqref{Eq:SFS_theory:WFS_General_Stat_pos} for $|\vxref(\vxo)-\vxo|$, termed the \emph{reference distance}.
...
\centering
	...
	\put(47,0){(b)}	
	\end{overpic}   
    \caption{2.5D synthesis of a 3D point source located at $\vxs = \posvec{3}{0}{-2}{0}$, radiating at $\omega_0 = 2\pi \cdot 1 \mathrm{krad}/s$ \fscom{1 kHz?} with the reference line set at $y_{\mathrm{ref}} = 1.5~\mathrm{m}$.
...
\importanteq{Linear WFS driv. fun. for a 3D point source}{
...
\importanteq{Temporal point source driv. fun.}{
d(\vxo, t) =  -
\sqrt{\frac{1}{2\pi c}}
...
\end{equation}
where $h(t) = \mathcal{F}^{-1}_t\left\{ \sqrt{\ti \omega} \right\}$ is the SSD compensation filter, performing half-derivation on the time history and $\hat{\vk}^{\mathrm{PW}} = \posvec{3}{\hat{k^{\mathrm{PW}}_x}}{k^{\mathrm{PW}}_y}{0}$ \fscom{check hats in equation} being a unit vector pointing into the plane wave propagation direction.
...
In the latter case the compact operator degenerates to the continuous eigenvalue domain instead of countable eigenvalues as presented in the following.
...
For practical applications the plane of synthesis is chosen to be the horizontal plane $z=0$ and reference the driving functions to a \emph{reference line} by setting $y = \yref$, requiring that for the virtual field $k_z(x,y,0) = 0$ holds.
...
,
\end{equation}
and in the spatial domain the inverse Fourier transform yields\fscom{in fact the calculus is performed using both H0 as prop and K0 as evanescent, see my diss from eq. 2.123}
\begin{equation}
...
A further large-argument approximation of the Hankel function yields the 2.5D WFS driving function for a 3D point source referencing the synthesis on a reference line, given by \eqref{eq:SFS_theory:WFS_point_source}, indicating the implicit solution constitutes a high frequency approximation for the explicit solution in case of a virtual point source.
...
The determination of a single spectral coefficient \fscom{avoid coefficient for continuous spectra} for the explicit solution requires the knowledge of the entire target field over the boundary surface in order to perform the spectral decomposition. 
...
In the following it is presented how the global solution can be transformed into a local one by the application of the stationary phase method.
...
The SPA therefore may be employed in order to give an approximate formulation for the 2.5D explicit driving functions purely in the spatial domain.
...
\end{align}
Since it is assumed, that $k_z^P(x,y) = k_z^G(x,y) = k_z = 0$ holds in the plane of investigation.
Therefore the stationary positions are found along a given $y$, where the local propagation direction of the virtual field and the Green's function matches to that of the spectral plane wave, defined by $k_x$.
...
\end{equation}
%
As it was discussed in Section \ref{Sec:SPA_for_Fourier}, generally speaking in the spatial inverse Fourier transform of an arbitrary wave field spectrum $\tilde{P}$ each wavenumber component $k_x$ will dominate one spatial position $x_0$, where the actual wavenumber component $k_x(x_0)$ coincides with the local wavenumber of the sound field $k_x^P(x_0)$.
...
\end{equation}
holds.
This definition relates the evaluation points $x^*_P$ and $x^*_G$ directly to the actual SSD coordinate $x_0$, therefore the dependency on the intermediate stationary wavenumber $k_x^*$ may be omitted, i.e., $x^*_P(k_x^*(x_0)) \rightarrow x^*_P(x_0)$ and $x^*_G(k_x^*(x_0)) \rightarrow x^*_G(x_0)$ is written. 
...
%	\hspace{-30mm}
    ...
    \caption{
	   ...
	   Furthermore, at $x^*_P(x_0)$ the local curvature of the translated Green's function is always greater, than that of the virtual field.\fscom{maybe exchange this with Fig.5 of the IEEE SDM WFS or indicate kP(xP) = kG(xP-xo)}
       } 
       \label{fig:SFS_theroy:explicit_sol_stationary_points}
  \end{minipage}
\end{figure}
%
The definitions for the forward and inverse transform stationary points completely define the evaluation points $x^*_P$, $x^*_G$ for a given secondary source $x_0$ independently of the spectral wavenumber:
...
Having found the stationary position for \eqref{Eq:inverse_transform_def} in order to apply the SPA one still needs the phase function's second derivative and its sign around the stationary position.
...
\end{multline}
The required rate of change of the stationary positions $x^{*'}_{P,k_x}(k_x)$ and $x^{*'}_{G,k_x}(k_x)$ can be obtained by differentiating their definition \eqref{eq:spectral_phase_first_derivative} with respect to $k_x$, resulting in\fscom{how/from where did you get there:?}
\begin{equation}
x^{*'}_{P,k_x}(k_x) = -\frac{1}{\phi^{P''}_{xx}(x^*_P(k_x),y)}, \hspace{5mm} x^{*'}_{G,k_x}(k_x) = -\frac{1}{\phi^{G''}_{xx}(x^*_G(k_x),y)}.
\end{equation}
\fscom{revision would be useful, consistent usage of either principle radii or curvatures or phase second derivatives, use text fragments from IEEE'18}
...
,
\end{equation}
\fscom{check notation of:} $\frac{1}{k\hat{k}^{P 2}_y} $
...
One important fact is pointed out here: although having derived the above driving functions in terms of a forward and an inverse spatial Fourier transform along a straight line, there is no restriction on the $y$-coordinate of the stationary point in \eqref{eq:SFS_theory:spatial_sdm} due to the local approximations involved: the $y$-coordinate might be $x_0$-dependent.
...
In the following an example is presented in order to demonstrate the validity of the spatial SDM driving functions for the synthesis of a virtual 3D point source.
...
In the following this relation is generalized by expressing the explicit driving functions for an arbitrary target sound field that is written in terms of pressure, measured along the SSD, revealing the general connection between the implicit and explicit solutions.

\section{Relation of implicit and explicit solutions}
\fscom{here a convenient link to the Application example \#1: Asymptotic evaluation of the Rayleigh integral in terms of calculus and notation would be nice}
...
\underbrace{\frac{\hat{k}_y^{P}(\vxo) P(\vxref(\vxo),\omega)}{G(\vxref(\vxo)-\vxo,\omega)}}_{\substack{\text{2D explicit}\\\text{driving function}}}.
\end{equation}
\fscom{check for consistency: $\hat{k}_y^{P}(\vxo)$ vs. $\hat{k}_y^{P}(\vxref(\vxo))$}
\begin{equation}
...
\end{equation}
In order to express the driving functions merely in terms of the involved quantities measured on the SSD, all the principal radii, the local wavenumber vector and the pressure of the target field has to be defined along the $y =0$ line.
...
\fscom{Therefore,} without the loss of generality the target pressure $P(\vxref(\vxo),\omega)$ can be formulated by the asymptotic Rayleigh integral \eqref{Eq:Asymptotic_Rayleigh_integral} with choosing the SSD position $\vxo$ as an independent variable, \fscom{therefore} \eqref{eq:SFS_theory:spatial_sdm_3} can be expressed as \fscom{a hint might be helpful: $\frac{P(\vxref(\vxo),\omega)}{G(\vxref(\vxo)-\vxo,\omega)}$ in D vs. rearranging $P(\vx,\omega) / G(\vx-\vxo^*(\vx),\omega)$ in P}
\begin{equation}
D(x_0,\omega) =
\small
...
The explicit solution allows the analytical description of aliasing artifacts, which can be directly applied to the results concerning WFS as well, due to the aforementioned asymptotic equivalence of the two methods.
As a complex application example for the equivalence of the explicit and implicit solutions, in the following the description and strategies for avoiding spatial aliasing are discussed in a unified manner.
...
In the foregoing steady-state analysis of wave fields was discussed, however spatial aliasing gains a simple physical interpretation when time domain analysis is considered.
...
A detailed investigation on the perception of spatial aliasing in WFS can be found in \fscom{missing refs, e.g. Wierstorf JASA and diss}.
In the following only objective description on the above phenomena is presented.

\begin{figure}
\centering
	\begin{overpic}[width = 1\columnwidth ]{Figures/SFS_theory/Spatial_alising.png}
	\end{overpic}   
    \caption{2.5D synthesis of a 3D point source located at $\vxs = \posvec{3}{0}{-2}{0}$, radiating at $\omega_0 =~2\pi \cdot 2 \mathrm{krad}/s$ (a), or emitting an impulse, bandlimited to $15~\mathrm{kHz}$.\fscom{indicate time instant}
...
In a recent publication it had been shown that by applying the unified WFS framework introduced in the present thesis, spatial aliasing artifacts can be directly described in the spatial domain \cite{Winter2018:GeometricModel} \fscom{maybe cite also the just submitted IEEE}.
...
Discretization of the SSD can be modeled mathematically as the sampling of the driving function with a sampling distance $\Delta x$ (i.e. the loudspeaker spacing):
\begin{equation}
...
\end{equation}
\fscom{$\frac{1}{\Delta x}$ makes the signal processing model consistent}
...
\centering
    ...
    The secondary source distance is set to $\Delta x = 10~\mathrm{cm}$, corresponding to the sampling wavenumber $k_{x,s} \approx 63~\mathrm{rad/m}$ and resulting in an aliasing frequency $\omega_a \approx 2\pi \cdot 1.7~\mathrm{rad/s}$. \fscom{1.7 kHz, xtick labels not correct}}
...
The discretization process of the driving functions is illustrated in the $k_x-\omega$ domain in Figure \ref{fig:SFS_theory:Aliased_spectrum} via the example of discretizing the driving function for a virtual point source, given by \eqref{eq:SFS_theory:WFS_point_source}.
...
This additive error manifests in additive wavefronts superimposed on to the non-aliased ideal wavefront (described by the $\eta = 0$ component in \eqref{Eq:SFS_theory:Aliased_field_sp}), \fscom{when speaking in the time-domain --- other formulation required}.
...
Hence figure \ref{fig:SFS_theory:Aliased_repr_field} (c) indicates \fscom{avoid suggest in general} that arbitrary directed plane waves with the angular frequency below the aliasing frequency can be synthesized with a discretized SSD.
...
A straightforward way to avoid spectral overlapping is to spatially bandlimit the driving function to the Nyquist wavenumber $k_{x,\mathrm{Nyq}} = k_{x,s}/2$ before discretization, i.e. by eliminating plane wave components that would cause spatial aliasing in the synthesis.
...
According to \eqref{eq:xP_xG_in_spatial_domain} a given wavenumber $k_x$ of the SDM driving function is dominated by that particular position of the SSD, where the local propagation direction of the virtual wave field coincides with that of the actual spectral plane wave, i.e. where $k_x^P(x_0) = k_x$ holds.
Hence, on a given angular frequency only those parts of the SSD will cause spectral overlapping, where the $x$-component of the virtual field's local wavenumber vector is above the Nyquist wavenumber\fscom{Nyquist wavenumber}, i.e. where
\begin{equation}
...
\centering
    ...
    Figure (c) shows spectrum of the synthesized field, measured along the reference line.\fscom{colorbars?}}
...
This formulation can be interpreted as a frequency dependent secondary source selection criterion.
Thus, the window function $w(\omega)$ in the general WFS driving function is made frequency dependent.
...
\small
  \begin{minipage}[c]{0.64\textwidth}
    ...
    This full-band SSD element is found, where the local propagation direction of the virtual field coincides with the SSD normal (i.e. the tangential local wavenumber component is zero).\fscom{is it helpful to also indicate the reference line? check tick labels}}
\label{fig:SFS_theory:AntiAliased_synthesis}   \end{minipage}
\end{figure}

Next the positions of antialiased synthesis is investigated: although spectral overlapping can be avoided by the presented strategy still, due to the non-directive nature of the 3D secondary point sources, lateral aliasing waves are present in the synthesized wave field.
...
Investigating Figure \ref{fig:SFS_theory:AntiAliased_spectrum} then indicates that at a given angular frequency $\omega$ with a given SSD element at $\vxo$ antialiased synthesis can be performed into directions for which $|k^P_t(\vxo)| < \frac{\pi}{\Delta x} \frac{c}{\omega}$ holds.
...
\small
  \begin{minipage}[c]{0.64\textwidth}
	...
	\fscom{how is the base band of the spectrum obtained? by changing the reference line? or only due to the anti alias filter?}
    }
...
The result of asymmetric anti-aliasing filtering is presented in figure \ref{fig:SFS_theory:AntiAliased_synthesis_asymm} (b), illustrating how anti-aliased synthesis may be optimized into an arbitrary direction.
This local increase of accuracy of the synthesis is \fscom{usually} referred to as \emph{Local Wave Field Synthesis (LWFS)}, being the subject of extensive study in the recent years. \fscom{give references}
Local increase of accuracy is \fscom{usually} achieved by some spatial bandwidth limitation strategy applied to the driving functions, \fscom{usually} performed in simple SSD geometries (e.g. linear, circular SSDs).
...
As presented in the foregoing, even besides ideal anti-aliasing filtering (spatial band-limitation) of the driving functions, still lateral aliasing components will be present following the virtual wavefront.
...
The far-field approximation of an extended radiator with a directive point source is given by \eqref{Eq:App:directive_monopole}.
...
Around the stationary point the directivity function is approximated by its value at the stationary point and the integral can be simplified as
\begin{equation}
...
\fscom{actually you don't really need this treatment, the signal processing model (see Start's and my diss) directly gives you the multplication of the post filter and green's function spectrum. I'd state this and cite and then move on with the following. Also: when performing a SPA on the integral, why only $\Theta$ pulling out of the integral and treating Greens function not as approximation?}
...
\small
  \begin{minipage}[c]{0.64\textwidth}
%	\begin{overpic}[width = 1\columnwidth]{Figures/SFS_theory/AntiAliased_spectrum_DirSSD.png}
%	\small
%	\put(2,1){(a)}
%	\end{overpic}   	
