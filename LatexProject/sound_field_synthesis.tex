...
In case of a non-homogeneous, harmonic spatial distribution a directivity factor is also present, given by $\sim \frac{c}{\ti k_y$,  with $\vk^P$ being the normalized local wavenumber vector, indicating that a surface of point sources generates larger pressure field into lateral directions than into the normal direction (see \eqref{eq:HF_approx:Greens_2D_Spectrum} and \eqref{Eq:HF_approx:H_det_Rayleigh} for its local approximation).
