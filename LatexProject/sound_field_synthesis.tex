...
\end{equation}
where $h(t) = \mathcal{F}^{-1}_t\left\{ \right \sqrt{\ti \omega} \}$ is the SSD compensation filter, performing a half-derivation on the time history and $\hat{\vk}^{\mathrm{PW}} = \posvec{3}{\hat{k^{\mathrm{PW}}_x}}{k^{\mathrm{PW}}_y}{0}$ being a unit vector pointing into the plane wave propagation direction.
...
Assume an infinite planar SSD along the $y = 0$ plane, degenerated from the geometry introduced for the Rayleigh integrals in the previous chapter, shown in Figure \ref{Fig:Theory:Rayleigh_geometry}.
...
Theoretically an arbitrary transfer function may be assigned for the SSD elements: as long the problem is well-conditioned---i.e. the spectrum of the transfer function does not exhibit zeros---unique driving functions may be derived applying the explicit methodology.
...
\end{equation}
The double inverse Fourier-transform may be carried out analytically, by taking the $y$-derivative of the Weyl's integral representation of a 3D point source (See \cite{Lalor1969} or \cite[(2.65)]{Williams1999}):
\begin{equation}
...
As it could be seen, the determination of a single spectral coefficient for the explicit solution requires the knowledge of the entire target field over the boundary surface in order to perform the spectral decomposition, therefore SDM is often termed as a \emph{global solution}.
In contrary, the implicit solution required the local field variables locally on the SSD position, at which the driving functions had to be expressed.
