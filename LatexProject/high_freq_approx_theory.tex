...
In order to derive integral representations better applicable for sound field synthesis, the application of approximate solutions are inevitable.
...
These approximations will be of crucial interest for finding the driving functions for general loudspeaker contours.
...
A wave field is \emph{divergent}, if it has a convex wavefront propagating away from a source distribution.
Similarly, the field is \emph{convergent} or \emph{focused}, if the concave wavefront propagates towards a focal point.
...
In the present case the wavefront normal is given by the (normalized) local wavenumber vector $\mathbf{k}^P(\mathbf{x})$ and the mean wavefront curvature $\overline{\kappa}^P(\vx)$ is given by
%
...
P(\vx,\omega) \approx A^P(\vxo,\omega) \te^{\ti ( \phi_0^P(\vxo,\omega) - \left< \vk^P(\vxo) \cdot \vx \right>} = P(\vxo,\omega) \te^{-\ti  \left< \vk^P(\vxo) \cdot \vx  \right>},
\end{equation}
with $\phi_0^P(\vxo,\omega) = \phi^P(\vxo,\omega) + \left< \vk^P(\vxo) \cdot \vxo \right>$.
...
The length of the wavenumber vector therefore decreases between the sources, where standing waves may occur, and increases to infinity on the parts where the amplitude vanishes and the phase changes rapidly due to destructive interference. \fpcom{x axis on the patch is incorrect}
}
...
The involved functions written in polar form reads \fpcom{Check the first therm, should be a derivative}
\begin{equation}
...
The stationary point $\vx^*(k_x)$ is found for a given $k_x$ \fpcom{and $k_z$}, where the gradient of the exponent is zero, thus where
\begin{align}
