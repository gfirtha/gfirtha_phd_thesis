...
termed the \emph{local wavenumber vector} of sound field $P$, being obviously the generalization of the plane wave wavenumber vector introduced by equation \eqref{Eq:Theory:PW_wavenumber_vec}\footnote{The negative sign ensures that the phase of the sound field decreases into the propagation direction, similarly to the plane wave case}.
...
\overline{\kappa}^P(\vx) =
\frac{\Dx \cdot \hat{\vk}^P(\vx)}{2} = - \frac{\Lx \phi^P(\vx,\omega)}{2 k} = - \frac{ \phi^{P''}_{xx}(\vx) + \phi^{P''}_{yy}(\vx) + \phi^{P''}_{zz}(\vx)}{2 k},
