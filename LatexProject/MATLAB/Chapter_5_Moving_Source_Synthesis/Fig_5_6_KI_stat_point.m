...
Delta = sqrt( (X-xs(1)-v*t).^2 + ( Y-xs(2) ).^2.*(1-M^2));
R = (M*(X-xs(1)-v*t)+Delta)/(1-M^2);

Delta0 = sqrt( (xr(1)-xs(1)-v*t).^2 + ( xr(2)-xs(2) ).^2.*(1-M^2));
R0 = (M*(xr(1)-xs(1)-v*t)+Delta0)/(1-M^2);
angle0 = 1i*(w*(t-R0/c)+pi/2);
field = exp(1i*w*(t-R/c)-angle0 )./Delta;

xse = ((xs(1)-M^2*xr(1)) - M*sqrt( (xr(1)^2-xs(1))^2+ (xr(2)-xs(2))^2*(1-M^2) )) /(1-M^2);

x0 = [ (xr(2)*xse-xs(2)*xr(1))/(xr(2)-xs(2)) , 0];
r2 = sqrt( (X-x0(1)).^2+(Y-x0(2)).^2);

angle1 = 1i*(k* sqrt( sum((xr-x0).^2) ) +pi/2);
field2 = exp(-1i*k*r2+angle1)./r2;

r0 = sqrt((xse-x0(1))^2+(xs(2)-x0(2))^2);
%%
x_ax = -1.5;
...
plot(R*cos(fi)+x0(1), R*sin(fi)+x0(2),'--k','LineWidth',1)
...
field2(y<0,:)=nan;
line([x(1),x(end)],[xs(2),xs(2)],'Color',[1 1 1]*0.5,'LineStyle','-','LineWidth',0.75)
plot(xse,  xs(2),'.k','MarkerSize',20)
plot(xs(1),xs(2),'.k','MarkerSize',20)
plot(xr(1),xr(2),'.k','MarkerSize',20)
plot(x0(1),x0(2),'.k','MarkerSize',20)
plot(xse+v*r0/c,  xs(2),'.k','MarkerSize',20,'Color',[1 1 1]*0.5)

[C2,~]= contour( x, y , real(field2), '--k');
set(gca,...
'XTickLabel','','YTickLabel','');
line([xse(1),xr(1)],[xs(2),xr(2)],'Color','black','LineStyle','--')
...
kxn_p = (x0(1)-xse(1))/sqrt(  (x0(1)-xse).^2 + (x0(2)-xs(2)).^2 );
kyn_p = (x0(2)-xs(2)) /sqrt(  (x0(1)-xse).^2 + (x0(2)-xs(2)).^2 );
...
ah5 = annotation('arrow',...
    'headStyle','cback2','HeadLength',headLength,'HeadWidth',headWidth,'LineWidth',1.5);
set(ah5,'parent',gca);
set(ah5,'position',[xs(1), xs(2), LineLength*1, LineLength*0]);    
...
print( '-r300' ,fullfile( '../..','Figures/Moving_Sources','KI_stat_point' ) ,'-dpng')