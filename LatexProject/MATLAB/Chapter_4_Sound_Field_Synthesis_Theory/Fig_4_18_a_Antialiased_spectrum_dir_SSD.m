...
Dkxs(isnan(Dkxs))=0;
%%
R = sqrt( X0.^2 + (yref-xs(2))^2 );
G0 = 1/(4*pi)*exp(-1i*W/c.*R)./R;   
Gkx = fftshift(fft(A.*G0,[],2),2)*dx;
...
pcolor(kx(1:q:end)/kxs,w(1:q:end)/(2*pi)/1e3,20*log10(abs(Pkx(1:q:end,1:q:end))))
hold on
...
print( '-r300',fullfile( '../..','Figures/SFS_theory','AntiAliased_spectrum_DirSSD' ) ,'-dpng')
